...
Feature: AISM-18
  ...
  Scenario Outline: Verify Filter is not saved if mandatory fields are left blank
    ...
    Examples: 
      | filter name                         | source       | keyword | comparator | keyword value |
      | AUT_Jira_ProjectIssue_RequiredField | Cisco Meraki | Country | Ends With  | JP            |
  ...
  Scenario Outline: Verify Filter is saved if non mandatory fields are left blank
    ...
    Examples: 
      | filter name                             | source       | keyword | comparator | keyword value |
      | AUT_Jira_ProjectIssue_NonRequiredFields | Cisco Meraki | Shop    | Ends With  | UQ            |

  @Regression
  Scenario Outline: Verify that Slack channel disappears if "Send ticket ID to Slack channel" checkbox is unchecked
    ...
    Examples: 
      | filter name                             |
      | AUT_Jira_ProjectIssue_NonRequiredFields |
